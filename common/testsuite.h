...
static void mavlink_test_sprayer_status(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_SPRAYER_STATUS >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_sprayer_status_t packet_in = {
        963497464,17443,151,218,29
    };
    mavlink_sprayer_status_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.total_ml = packet_in.total_ml;
        packet1.flow_rate = packet_in.flow_rate;
        packet1.enabled = packet_in.enabled;
        packet1.power = packet_in.power;
        packet1.tank_level = packet_in.tank_level;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_SPRAYER_STATUS_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_SPRAYER_STATUS_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sprayer_status_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_sprayer_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sprayer_status_pack(system_id, component_id, &msg , packet1.enabled , packet1.power , packet1.tank_level , packet1.flow_rate , packet1.total_ml );
    mavlink_msg_sprayer_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sprayer_status_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.enabled , packet1.power , packet1.tank_level , packet1.flow_rate , packet1.total_ml );
    mavlink_msg_sprayer_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_sprayer_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sprayer_status_send(MAVLINK_COMM_1 , packet1.enabled , packet1.power , packet1.tank_level , packet1.flow_rate , packet1.total_ml );
    mavlink_msg_sprayer_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_sprayer_status(system_id, component_id, last_msg);
